...
// Copyright (c) 2017-2018 The Timicoin developers
...
#if defined(NDEBUG)
#error "Timicoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utmc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zTmc with serial %s is already in the block %d\n", __func__,
    ...
    
    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTmc spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zTMC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if(nHeight == 1) {
        ...
        nSubsidy = 60001 * COIN;
    } else if (nHeight < 8040 && nHeight > 2) {
        nSubsidy = 250 * COIN;
    } else if (nHeight < 10000 && nHeight >= 8040) {
        nSubsidy = 100 * COIN;
    }
    else if (nHeight < (Params().NetworkID() == CBaseChainParams::TESTNET ? 145000 : 20000) && nHeight >= 10000) {
        nSubsidy = 80 * COIN;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 20000) {
        nSubsidy = 40 * COIN;
    } else if (nHeight <= 3999999 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 39 * COIN;
    } else if (nHeight <= 7999999 && nHeight >= 4000000) {
        nSubsidy = 38 * COIN;
    } else if (nHeight <= 13999999 && nHeight >= 8000000) {
        nSubsidy = 37 * COIN;
    } else if (nHeight <= 17999999 && nHeight >= 14000000) {
        nSubsidy = 36 * COIN;
    } else if (nHeight <= 18499999 && nHeight >= 18000000) {
        nSubsidy = 35 * COIN;
    } else if (nHeight <= 18653960 && nHeight >= 18500000) {
        nSubsidy = 34 * COIN;
    } else if (nHeight <= 18653963 && nHeight >= 18653961) {
        nSubsidy = 25 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Timicoin
...
{
    RenameThread("timicoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTMCMinted()
{
...
void RecalculateZTMCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTMC supply
...
bool RecalculateTMCSupply(int nHeightStart)
{
...
bool UpdateZTMCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTMCSupply(Params().Zerocoin_StartHeight());
    }
    
    //Track zTMC money supply in the block index
    if (!UpdateZTMCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTMC supply for block=%s height=%d", __func__,
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTmcSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TMC to zTMC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTmc spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTmc serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTmc mints and %d zTmc spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Timicoin: We use certain sporks during IBD, so check to see if they are
