// Copyright (c) 2017 The Timicoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "ztmccontroldialog.h"
#include "ui_ztmccontroldialog.h"
...
std::list<std::string> ZTmcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTmcControlDialog::listMints;

ZTmcControlDialog::ZTmcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTmcControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTmcControlDialog::~ZTmcControlDialog()
{
    delete ui;
}

void ZTmcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTmcControlDialog::updateList()
{
...
void ZTmcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTmcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTmc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTmcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTmcControlDialog::GetSelectedMints()
{
...
void ZTmcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
