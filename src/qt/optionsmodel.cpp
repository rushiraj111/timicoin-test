...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TMC);
    ...
    if (!settings.contains("nAnonymizeTmcxAmount"))
        settings.setValue("nAnonymizeTmcxAmount", 1000);

    nAnonymizeTmcxAmount = settings.value("nAnonymizeTmcxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTmcxAmount"))
        SoftSetArg("-anonymizetimicoinamount", settings.value("nAnonymizeTmcxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in timicoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTmcxAmount:
            return QVariant(nAnonymizeTmcxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTmcxAmount:
            nAnonymizeTmcxAmount = value.toInt();
            settings.setValue("nAnonymizeTmcxAmount", nAnonymizeTmcxAmount);
            emit anonymizeTmcxAmountChanged(nAnonymizeTmcxAmount);
            break;
        case CoinControlFeatures:
